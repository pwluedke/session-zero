...
// ── Settings ───────────────────────────────────────────────────────────────
test('Why? button is hidden when ai_enabled is false', async ({ page }) => {
  const main = new MainPage(page);
  await page.route('**/api/me', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ id: 1, display_name: 'User', email: 'u@test.com', avatar_url: null, role: 'user', ai_enabled: false }),
  }));
  await page.reload();
  await page.waitForLoadState('networkidle');
  await main.findGames();
...
test('Why? button is visible when ai_enabled is true', async ({ page }) => {
  const main = new MainPage(page);
  await page.route('**/api/me', route => route.fulfill({
    status: 200,
    contentType: 'application/json',
    body: JSON.stringify({ id: 1, display_name: 'User', email: 'u@test.com', avatar_url: null, role: 'user', ai_enabled: true }),
  }));
  await page.reload();
  await page.waitForLoadState('networkidle');
  await main.findGames();
  await expect(page.getByTestId('why-btn').first()).toBeVisible();
});
